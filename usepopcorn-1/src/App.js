  ...
  useEffect(
    function () {
      if (query.length < 3) {
        ...
        setIsLoading(false);
        setError("");
        console.log("Cleared movies because query is too short");
        return;
      }
    ...
    <div>
      ...
      <Main>
        <Box>
          {isLoading && <Loader />}
          {!isLoading && !error && (
            <List list={movies} onSelectedMovie={handleSelectedMovie} />
          )}
...
function List({ list, onSelectedMovie }) {
  return (
    <ul className="list list-movies" onClick={onSelectedMovie}>
      {list?.map((movie) => (
        <Movie
          movie={movie}
          key={movie.imdbID}
